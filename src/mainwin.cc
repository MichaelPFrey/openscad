...
void MainWindow::updateCamera(const FileContext &ctx)
{
	...
	if (vpr->getVec3(x, y, z, 0.0)){
		...
		PRINTB("WARNING: Unable to convert $vpr=%s to a vec3 or vec2 of numbers", vpr->toString());
	}
	...
	if (vpt->getVec3(x, y, z, 0.0)){
		...
		PRINTB("WARNING: Unable to convert $vpt=%s to a vec3 or vec2 of numbers", vpt->toString());
	}
	...
	if (vpd->type() == Value::ValueType::NUMBER){
		...
		PRINTB("WARNING: Unable to convert $vpd=%s to a number", vpd->toString());
	}
}
